...
class controlmaestros2 extends plantilla
{
...
<script>
  ...
  function buscar()
  {
    ...
    {
      var ur = "controlmaestros2.php?pardebusqueda="+$("#pald").val()
...
function buscarE($par, $val)
{
  ...
  if($par=="Fecha nac.")
  {
    $qry = sprintf("SELECT * FROM maestro WHERE FechaNacimiento = '%s'",$val);
  }//end else if
  else
  {
    $qry = sprintf("SELECT * FROM maestro WHERE %s = '%s'",$par,$val);
  }//end if
